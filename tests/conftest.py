...
def run_octave(command: str) -> str:
    ...
    """
    # Ensure paths are correct based on the repo root (its + mex so Octave finds its_* and nn_*)
    matlab_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'matlab'))
    full_command = (
        f"addpath('{matlab_dir}'); addpath('{matlab_dir}/its'); addpath('{matlab_dir}/mex'); {command}"
    )
