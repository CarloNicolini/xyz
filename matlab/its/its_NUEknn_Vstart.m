...
function out=its_NUEknn_Vstart(data,j,candidates,nnk,num_rnd,minshift,alpha_rnd,metric,V_start)

if ~exist('V_start','var'), V_start=[]; end
...
while exitcrit==0
    ...
    for i=1:size(candidates,1)
        if isnan(candidates(i,3)) % test i-th candidate, only if not already included in V
            ...
            c_mi(i)=its_CMIknn(B,nnk,metric); % CONDITIONAL mutual information knn
        end
    end    
    ...
    CMI(k-1)=its_CMIknn(B,nnk,metric); % conditional mutual information of the selected term (nn estimate)
    ...
    for is=1:num_rnd
        ...
        CMIs(is)=its_CMIknn(Bs,nnk,metric);
    end
...
cmiOK=cmiOK';
L=k-2+size(V_start,1); %optimal embedding length
