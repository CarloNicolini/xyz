% Function from Dimitris

function [xM, flagexplode] = coupledhenonmaps2(m,c,n,aV,x0M)
...
% modified to make sure that does not explode
xMmin=min(xM(:));
xMmax=max(xM(:));
deltaxM=abs(xMmax-xMmin);
if deltaxM>10^4
    warning('Generated Henon maps seem to explode...');
    flagexplode=1;
else
    flagexplode=0;
end
